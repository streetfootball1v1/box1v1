...
// Ждём пока загрузится библиотека Supabase
let db;
...
function initializeSupabaseService() {
  ...
  // Проверяем что конфиг есть
  if (!SUPABASE_CONFIG || !SUPABASE_CONFIG.url || !SUPABASE_CONFIG.publishableKey) {
    console.error('❌ Конфигурация Supabase не настроена');
    return null;
  }
  ...
  class SupabaseService {
    constructor() {
      try {
        ...
        console.log('✅ SupabaseService инициализирован');
      } catch (error) {
...
// Инициализируем когда всё загрузится
document.addEventListener('DOMContentLoaded', function() {
  // Даём время на загрузку библиотек
  setTimeout(() => {
    db = initializeSupabaseService();
